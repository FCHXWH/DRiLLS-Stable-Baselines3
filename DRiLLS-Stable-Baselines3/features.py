...
def log(message):
    print('[DRLLS {:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now()) + "] " + message)
...
def extract_features(design_file, yosys_binary='yosys', abc_binary='abc'):
    ...
    '''
    stats = {};
    stats = yosys_stats(design_file,yosys_binary,stats);
    stats = abc_stats(design_file,abc_binary,stats);
    #manager = Manager()
    #stats = manager.dict()
